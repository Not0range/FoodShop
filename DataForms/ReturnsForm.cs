    ...
    public partial class ReturnsForm : Form
    {
        ...
        private void LoadData()
        {
            using (var sql = new SqlConnection(Program.sql.ConnectionString))
            {
                ...
                var cmd = new SqlCommand("Select t1.*, t3.Название as Товар from Возвраты t1 inner join Поставки t2 on t1.[ID товара] = t2.ID " +
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (form.ShowDialog() == DialogResult.OK)
            {
                using (var sql = new SqlConnection(Program.sql.ConnectionString))
                {
                    ...
                    var cmd = new SqlCommand("Insert into Возвраты values (@Product, @Date, @Count, @Reason)", sql);
                    ...
                    cmd.Parameters.AddWithValue("@Reason", form.reasonBox.Text);
                    cmd.ExecuteNonQuery();
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            form.reasonBox.Text = dataGridView1.SelectedRows[0].Cells[5].Value.ToString();
            ...
            if (form.ShowDialog() == DialogResult.OK)
            {
                using (var sql = new SqlConnection(Program.sql.ConnectionString))
                {
                    ...
                    var cmd = new SqlCommand("Update Возвраты set [ID товара] = @Product, [Дата и время] = @Date, Количество = @Count, [Причина] = @Reason where ID = @ID", sql);
        ...
        private void button3_Click(object sender, EventArgs e)
        {
                ...
                MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
            {
                ...
                using (var sql = new SqlConnection(Program.sql.ConnectionString))
                {
                    ...
                    try
                    {
                        var cmd = new SqlCommand("Delete from Возвраты where ID = @ID", sql);
